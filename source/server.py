...
# 处理发送文件请求函数
def handle_file(connection, address):
    global chat_user  # 聊天对象
    # 调用函数获取聊天对象
    chat_user = recv_all_string(connection)
    # 调用函数获取文件名
    file_path = recv_all_string(connection)
    file_name = os.path.basename(file_path)
    # print('server收到发送文件请求：'+file_path) 
    if chat_user == "【群聊】":  # 如果聊天对象是群聊
        # 给所有在线用户发送文件
        for c in online_connection:
            # 先发送文件标记
            send_string_with_length(c, "#!file#!")
            # 发送聊天对象标记，为群聊
        ...
        for c in online_connection:  # 寻找聊天对象
            if connection_user[c] == chat_user:  # 从字典中查找对象，查到则执行下面语句
                ...
                send_string_with_length(c, connection_user[connection])
                # 发送文件名
                # send_string_with_length(c, file_name)
                # 发送文件内容
                # 定义定义文件信息。128s表示文件名为128bytes长，l表示一个int或log文件类型，在此为文件大小
                fileinfo_size = struct.calcsize('128sl')
                # 定义文件头信息，包含文件名和文件大小
                ...
                # 给自己发送消息
                send_string_with_length(connection, "#!file#!")
                send_string_with_length(connection, "private_chat")
                send_string_with_length(connection, connection_user[connection])
                # send_string_with_length(connection, file_name)
                # 定义定义文件信息。128s表示文件名为128bytes长，l表示一个int或log文件类型，在此为文件大小
                fileinfo_size = struct.calcsize('128sl')
                # 定义文件头信息，包含文件名和文件大小
                fhead = struct.pack('128sl', os.path.basename(file_path).encode('utf-8'), os.stat(file_path).st_size)
...
def handle(connection, address):
    try:
        while True:
            ...
            elif request_type == "5":  # 发送文件请求
                print("开始处理发送文件请求")
                no_go = handle_file(connection, address)
            if not no_go:
                break
