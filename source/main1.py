...
def private_talk(self):
    ...
    index = indexs[0]
    # print("chat_user"+main_frame.friend_list.get(index))
    if index > 0 and main_frame.friend_list.get(index) != " ":
        chat_user = main_frame.friend_list.get(index)  # 获取点击的用户名
        # 修改标题名称
        if chat_user == '【群聊】':  # 如果聊天对象为群聊，设置如下标题
            title = "在线用户         欢迎进入聊天室：" + main_frame.user_name + "                                    "
...
def send_file(file_path):
    ...
    if file_path == "":
        ...
        return
    print(file_path)
    # 调用在此类创建的chat_client模块的对象的实例方法发送文件给服务器
    client.send_file(file_path, chat_user)
...
def go_to_main_panel(user_name):
    ...
    main_frame = main_panel.MainPanel(user_name, send_message, send_mark, send_file, refurbish_user, private_talk, close_main_window)
    # 创建子线程专门负责接收并处理数据
    ...
    while True:
        try:
            ...
            if data_type == "#!onlinelist#!":
                ...
                user = client.recv_all_string()
                print("user: " + user)
                file_name, file_path = client.recv_all_file()
                # print("file_path: " + file_path)
                # print("file_name: " + file_name)
                # 对象调用实例方法显示文件
                main_frame.show_send_file(user, file_name, file_path, chat_flag)
...
def go_to_login_panel():
    ...
    client = client.ChatSocket()  # 创建client模块中的客户端连接服务器的socket对象
    global login_frame # 声明全局变量，可以在其他函数使用
    login_frame = login_panel.LoginPanel(handding_login, handding_register, close_login_window)  # 可以把login_pannel模块的事件绑定在这几个函数上
