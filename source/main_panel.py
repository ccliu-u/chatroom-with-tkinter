...
class MainPanel:
    ...
    def __init__(self, user_name, send_message, send_mark, send_file, refurbish_user, private_talk, close_main_window):
        # 初始化参数实例变量
        ...
        self.face = []  # 存储头像列表
        self.pic_main = []  # 面板图片列表
        self.pic_rec = []  # 聊天记录图片列表
    ...
    def show_main_panel(self):
        ...
        p1 = Image.open('表情包/微信表情1.png')
        p1 = p1.resize((30, 30), Image.ANTIALIAS)
        self.p1 = ImageTk.PhotoImage(p1)
        
        p2 = Image.open('表情包/微信表情2.png')
        p2 = p2.resize((30, 30), Image.ANTIALIAS)
        self.p2 = ImageTk.PhotoImage(p2)

        p3 = Image.open('表情包/微信表情3.png')
        p3 = p3.resize((30, 30), Image.ANTIALIAS)
        self.p3 = ImageTk.PhotoImage(p3)

        p4 = Image.open('表情包/微信表情4.png')
        p4 = p4.resize((30, 30), Image.ANTIALIAS)
        self.p4 = ImageTk.PhotoImage(p4)

        p5 = Image.open('表情包/微信表情5.png')
        p5 = p5.resize((30, 30), Image.ANTIALIAS)
        self.p5 = ImageTk.PhotoImage(p5)

        p6 = Image.open('表情包/微信表情6.png')
        p6 = p6.resize((30, 30), Image.ANTIALIAS)
        self.p6 = ImageTk.PhotoImage(p6)

        p7 = Image.open('表情包/微信表情7.png')
        p7 = p7.resize((30, 30), Image.ANTIALIAS)
        self.p7 = ImageTk.PhotoImage(p7)

        p8 = Image.open('表情包/微信表情8.png')
        p8 = p8.resize((30, 30), Image.ANTIALIAS)
        self.p8 = ImageTk.PhotoImage(p8)

        p9 = Image.open('表情包/微信表情9.png')
        p9 = p9.resize((30, 30), Image.ANTIALIAS)
        self.p9 = ImageTk.PhotoImage(p9)

        p10 = Image.open('表情包/微信表情10.png')
        p10 = p10.resize((30, 30), Image.ANTIALIAS)
        self.p10 = ImageTk.PhotoImage(p10)

        # 按钮图片，把图片转换为PhotoImage
        img1 = Image.open('button/表情按钮.png')
        img1_resized = img1.resize((25, 25), Image.ANTIALIAS)
        self.p11 = ImageTk.PhotoImage(img1_resized)

        img2 = Image.open('button/聊天记录按钮.png')
        img2_resized = img2.resize((25, 25), Image.ANTIALIAS)
        self.p12 = ImageTk.PhotoImage(img2_resized)

        img3 = Image.open('button/文件按钮.png')
        img3_resized = img3.resize((25, 25), Image.ANTIALIAS)
        self.p13 = ImageTk.PhotoImage(img3_resized)
        ...
        # 设置文本标签和位置
        self.label1 = Label(self.main_frame, text="在线用户         欢迎进入聊天室：" + self.user_name + "                                    ", 
                            font=("黑体", 16), bg="#00BFFF", fg="white")
        ...
        self.friend_list = Listbox(self.main_frame, selectmode=NO, listvariable=friend_list_var,
                                   bg="#F8F8FF", fg="#0052D9", font=("等线", 14),
        ...
        button1 = Button(self.main_frame, command=lambda: self.send_message(self), text="发送", bg="#00BFFF",
                         fg="white", width=11, height=2, relief="flat", font=('黑体', 12))
        button1.place(x=750, y=640)

        # 设置关闭窗口按钮及位置，事件处理函数为close_main_window
        button2 = Button(self.main_frame, text="关闭", bg="white", fg="black", width=11, height=2,
                              font=('黑体', 12), relief="flat", command=self.close_main_window)
        button2.place(x=630, y=640)
        ...
        # 设置文件按钮及位置，事件处理为select_file实例方法
        botton6 = Button(self.main_frame, command=self.select_file, image=self.p13, relief=FLAT, bd=0)
        botton6.place(x=286, y=525)
        ...
        botton5 = Button(self.main_frame, command=self.refurbish_user, text="刷新在线用户", bg="#00BFFF", fg="white",
                         width=13, height=2, relief="flat", font=('黑体', 12),)
    ...
    def create_window(self):
        ...
        # 设置文本标签
        title_lable = Label(top1, text="聊天记录", font=('黑体', 16, 'bold'),
                            fg="white", bg="#00BFFF")
        # 设置文本在窗口的位置
        ...
        botton = Button(top1,  text="清空聊天记录", command=self.clear_chatting_records, bg="#00BFFF",
                        fg="white", width=12, height=2, relief="flat", font=('黑体', 11))
    ...
    def show_chatting_records(self):
        ...
        while True:
            ...
            if content != "":  # 如果不为空则在文本框最后一行插入
                if content.strip() in self.dic:  # 如果读取到的内容是表情标记，则插入表情
                    self.chatting_records.image_create(END, image=self.dic[content.strip()])
                    self.chatting_records.insert(END, "\n")
                elif content.strip() == "[图片]**":  # 如果读取到的内容是图片标记，则插入图片
                    content = f.readline()
                    self.img_rec = Image.open(content.strip())  # 打开文件
                    MAX_WIDTH = 100  # 最大宽度
                    MAX_HEIGHT = 100  # 最大高度
                    width, height = self.img_rec.size
                    if width > MAX_WIDTH or height > MAX_HEIGHT:
                        # 计算等比例缩放后的尺寸
                        ratio = min(MAX_WIDTH / width, MAX_HEIGHT / height)
                        new_width = int(width * ratio)
                        new_height = int(height * ratio)
                        self.img_rec = self.img_rec.resize((new_width, new_height), Image.ANTIALIAS)
                    self.img_tk_rec = ImageTk.PhotoImage(self.img_rec)
                    self.pic_rec.append(self.img_tk_rec)
                    self.chatting_records.image_create(END, image=self.pic_rec[-1])  # 显示图片
                    self.chatting_records.insert(END, "\n")
                else:
                    ft = tf.Font(family='微软雅黑', size=13)  # 设置字体样式和大小变量
                    # 设置颜色和字体样式及大小
                    self.chatting_records.tag_config("tag_9", foreground="#000000", font=ft)
    ...
    def clear_chatting_records(self):
        ...
        # 打开聊天记录文件，以覆盖的形式写入内容
        a = open("./chatting_records/" + self.user_name + ".txt", 'w')
    ...
    def express(self):
        ...
        if self.ee == 0:
            ...
            # 设置表情包的位置
            self.b1.place(x=207, y=490)
            self.b2.place(x=240, y=490)
            self.b3.place(x=273, y=490)
            self.b4.place(x=306, y=490)
            self.b5.place(x=339, y=490)
            self.b6.place(x=207, y=455)
            self.b7.place(x=240, y=455)
            self.b8.place(x=273, y=455)
            self.b9.place(x=306, y=455)
            self.b10.place(x=339, y=455)
        else:
            # 标记ee为0则销毁所有表情按钮
            self.ee = 0
            self.b1.destroy()
    ...
    def refresh_friends(self, online_number, names):
        ...
        for name in names:  # 循环插入在线用户
            self.friend_list.insert(0, f"{name}")
        self.friend_list.insert(0, "【群聊】")  # 在第二行插入群聊
        self.friend_list.itemconfig(0, fg="#0052D9")  # 设置群聊字体颜色
        self.friend_list.insert(0, " ")  # 插入一个空行
    ...
    def show_send_message(self, user_name, content, chat_flag):
        ...
        if content == '* 系统提示: ' + user_name + ' 加入聊天室':  # 加入聊天室标记处理
            ...
            if chat_flag == "group_chat":  # 如果标记是群聊标记，则自己的消息用蓝色
                ...
                self.message_text.tag_config("tag_4", foreground="#0052D9", font=ft)
        ...
        if content in self.dic:  # 判断消息是否为表情标记
            ...
            # 保存图片，类型为png
            self.out1.save(f"./头像/{user_name}头像1.png", 'png')
            time.sleep(0.5)  # 给修改图片大小以及保存修改后的图片留时间缓存
            # 把头像转化为PhotoImage
            self.face.append(PhotoImage(file=f"./头像/{user_name}头像1.png"))  # 把头像图片加入到列表中
        ...
        # 内容是消息的处理
            ...
            # 保存图片，类型为png
            self.out2.save(f"./头像/{user_name}头像2.png", 'png')
            time.sleep(0.5)
            self.face.append(PhotoImage(file=f"./头像/{user_name}头像2.png"))
    ...
    # 显示文件
    def show_send_file(self, user_name, file_name, file_path, chat_flag):
        print("开始显示文件")
        self.message_text.config(state=NORMAL)   # 设置消息框可编辑
        # 设置发送的消息的用户名和时间变量
        title = user_name + " " + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + "\n"
        if user_name == self.user_name:  # 如果发送消息的用户是自己
            if chat_flag == "group_chat":  # 如果标记是群聊标记，则自己的消息用蓝色
                ft = tf.Font(family='微软雅黑', size=12)
                self.message_text.tag_config("tag_4", foreground="#0052D9", font=ft)
                self.message_text.insert(END, title, 'tag_4')
        ...
        if file_name.lower().endswith(image_extensions):
            self.img3 = Image.open(file_path)
            MAX_WIDTH = 100  # 最大宽度
            MAX_HEIGHT = 100  # 最大高度
            ...
            if width > MAX_WIDTH or height > MAX_HEIGHT:
                ...
                new_width = int(width * ratio)
                new_height = int(height * ratio)
                self.img3 = self.img3.resize((new_width, new_height), Image.ANTIALIAS)
                
            self.img_tk3 = ImageTk.PhotoImage(self.img3)
            self.pic_main.append(self.img_tk3)
            self.message_text.image_create(END, image=self.pic_main[-1])  # 显示图片
            self.message_text.insert(END, "\n")
            self.message_text.config(state=DISABLED)
            # 滚动到最底部
            self.message_text.see(END)
            # 保存聊天记录
            self.sava_chatting_records("[图片]**\n")
            self.sava_chatting_records(file_path+'\n')
            self.sava_chatting_records("------------------------------------------------------------------------------\n")
        else:
            ft = tf.Font(family='微软雅黑', size=13)
            self.message_text.tag_config("tag_8", foreground="#000000", font=ft)
            self.message_text.insert(END, f"[收到文件：{file_name}，请前往recv_file文件夹查看]", 'tag_8')
            self.message_text.insert(END, "\n")
            self.message_text.config(state=DISABLED)
            # 滚动到最底部
            self.message_text.see(END)
            # 保存聊天记录
            self.sava_chatting_records("[文件]**\n")
            self.sava_chatting_records(file_path+'\n')
            self.sava_chatting_records("------------------------------------------------------------------------------\n")
